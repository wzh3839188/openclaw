...
export function createBrowserTool(opts?: {
  ...
  return {
    ...
    description: [
      ...
      'Chrome extension relay needs an attached tab: user must click the OpenClaw Browser Relay toolbar icon on the tab (badge ON). If no tab is connected, ask them to attach it. The extension auto re-attaches after navigation (retries for ~24s for heavy SPAs). For fully automated browsing with no manual attach, use profile="openclaw" (OpenClaw-controlled browser).',
