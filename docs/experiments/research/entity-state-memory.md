...
针对「用户 = 实体、习惯/术语→做法 = 属性」的场景，可在上述通用 entities + states 之上增加一层 **user_attributes**（或等价表/结构）。属性类型与上文「用户实体可拥有的属性类型」对应，用 **type** 区分注入与匹配策略：
...
- **Phase 0（约定 + 手工/模型维护）**  
  - 在 workspace 内约定一个 **用户习惯文件**（如 `memory/user-habits.md` 或 `USER_HABITS.md`），格式可为：每个习惯一块，含 **名称**、**触发词/短语**（用于匹配用户 query）、**做法说明**（命中后要带给 LLM 的原文）。  
  - 用户或模型在对话中“调教”一次后，由模型或用户把该条习惯追加/更新到此文件。  
  - **匹配与注入**：在 gateway/agent 的“组请求、调 LLM”之前增加一步：读取该文件，用当前 user message 对“触发词/短语”做简单匹配（关键词包含或正则）；若有命中，将对应“做法说明”拼成一段，注入当轮 system 或单独 context 块。  
  - 不依赖新表结构，先验证“query 前匹配 → 注入”是否解决“新 session 又忘”的问题。

- **Phase 1（结构化存储 + 可配置触发）**  
  - 将习惯存入结构化存储（SQLite 表 `user_attributes` 或等价），字段包含：`attribute_key`、`trigger`（支持多关键词/正则）、`procedure`。  
  - 匹配逻辑可配置（如只做关键词 vs 支持简单正则），注入长度上限可配置（如 `userHabitsInject.maxChars`），避免习惯过多导致 context 膨胀。  
  - 提供工具或命令：用户/模型可增删改“我的习惯”，并持久化到上述存储。
